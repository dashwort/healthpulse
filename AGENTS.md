...
- Administrator access activity retains web sign-in outcomes and approved Android authorizations for seven days. It may store an attributable active user ID, source IP address, and bounded user agent, but never credentials, provider error text, request content, or health data. When deployed through Cloudflare Tunnel, trust `CF-Connecting-IP` only from explicitly configured private tunnel networks.
